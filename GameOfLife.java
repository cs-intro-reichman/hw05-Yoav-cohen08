...
public class GameOfLife {
	...
	public static int[][] evolve(int[][] board) 
	{
		...
		for(int i = 0; i < rows; i++)
		{
			...
			{
				if((i == 0) || (j == 0) || (i == rows - 1) || (j == cols - 1)) //Zeros around the game board.
				{
					newBoard[i][j] = 0;
				}
				else
	...
	public static int cellValue(int[][] board, int i, int j) 
	{
		...
		if((board[i][j] == 1) && (count <= 1)) return 0;                  // If the cell is alive (equals 1) and has fewer than two live neighbors, it dies (becomes 0).
		else if((board[i][j] == 1) && (count == 2)) return 1;			  // If the cell is alive and has two or three live neighbors, it remains alive.
			else if((board[i][j] == 1) && (count == 3)) return 1;	
				else if((board[i][j] == 1) && (count > 3)) return 0;	  // If the cell is alive and has more than three live neighbors, it dies.
					else if((board[i][j] == 0) && (count == 3)) return 1; // If the cell is dead and and has three live neighbors, it becomes alive.
						else return (board[i][j]);						  // Otherwise the cell does not change. 	
	}
	...
	{
		int count = 0;
		count += board[i - 1][j - 1]; //Left top corner
		count += board[i - 1][j]; // Above.
		count += board[i - 1][j + 1]; // Right top corner.
		count += board[i][j - 1]; //Left.
		count += board[i][j + 1]; //Right.
		count += board[i + 1][j - 1]; // Left bottom corner.
		count += board[i + 1][j]; //Under.
		count += board[i + 1][j + 1]; //Right bottom corner.
		return count;
	}
